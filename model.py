...
class point3:
    ...
    def ofLength(self, length):
        if self.length == 0:
            raise ValueError("In Point3 attepted to normalize a zero vector")
...
class Swordsman(Character):
    ...
    def normalAttack(self):
        ...
        self.round.addEntity(
            Projectile(self.pos, 100, "sword", self.wrange,
                       self.team, self.damage, 0.2)
        )
...
class Wizard(Character):
    ...
    def shoot(self, shootPos):
        ...
        explosProjectile = Projectile(
            self.pos, 30, "shoot", self.shootRadius, self.team, self.damage, 1)
...
class Round:
    ...
    def removeExpired(self) -> None:
        self.__entities = [e for e in self.__entities if not (
            type(e) == Projectile and e.life < 0)]
...
class Agent:
    ...
    def makeMove(self):
        ...
        enemy = [en for en in self.round.entities if isinstance(
            en, Character) and en.team != self.character.team][0]
        # calculate actions
        if self.AI is None:
            return
